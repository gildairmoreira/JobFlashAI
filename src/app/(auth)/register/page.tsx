...
export default function RegisterPage() {
  ...
  const handleEmailRegister = async (e: React.FormEvent) => {
    ...
    // Sanitização básica contra injeção de HTML no nome
    ...
    try {
      ...
      if (error) {
        toast.error(error.message || "Erro ao criar conta. Verifique os dados fornecidos.");
        setIsLoading(false);
        return;
      }
    ...
    >
      <div className="space-y-2 text-center md:text-left">
        ...
        <p className="text-stone-500 dark:text-stone-400 text-sm">
          Junte-se a milhares de profissionais. É de graça e leva 10 segundos.
        </p>
      </div>
      ...
      <p className="text-center text-xs text-stone-400 dark:text-stone-500 px-4 mt-8">
        Ao continuar, você concorda com nossos{" "}
        <a href="/terms" className="underline hover:text-stone-900 dark:hover:text-stone-100 transition-colors">Termos de Uso</a> e{" "}
